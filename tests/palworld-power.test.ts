...
const server = { uid: 'test', name: 'Pal Fixture', dataPath: '/fixture/test', containerId: 'owned', allocations: [{ purpose: 'rest', port: 32502 }] };
...
it('uses applied credentials and the private allocated endpoint, saves before shutdown, then verifies clean exit', async () => {
  ...
  expect(fetcher.mock.calls.map((call) => (call as unknown[])[0])).toEqual(['http://pal-fixture-test:32502/v1/api/save', 'http://pal-fixture-test:32502/v1/api/shutdown']);
