...
export async function stopPalworld(server: { uid: string; dataPath: string; containerId: string; allocations: { purpose: string; port: number }[] }, runtime: Pick<RuntimeDriver, 'status'>) {
  ...
  const host = runningInContainer() ? `${config.brand.resourcePrefix}-${server.uid}` : '127.0.0.1';
