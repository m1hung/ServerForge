...
/** Router discovery is deliberately restricted to literal LAN IPv4 addresses. */
export function validateGatewayUrl(value: string): URL {
  ...
  if (
    ...
    throw new Error('Enter an HTTP or HTTPS router URL using its LAN IPv4 address.');
  }
  return url;
}
...
async function limitedText(response: Response): Promise<string> {
  ...
  try {
    while (true) {
      ...
      if (done) break;
      size += value.length;
      if (size > 1024 * 1024) throw new Error('Router response was too large.');
      chunks.push(value);
    }
  } finally {
  ...
  if (/<!DOCTYPE|<!ENTITY/i.test(text)) throw new Error('Unsupported router XML.');
  return text;
}
...
export async function discoverGateway(timeoutMs = 3000): Promise<Gateway | null> {
  ...
  const gateways = await Promise.all(
    locations.slice(0, 8).map((location) => describeGateway(location).catch(() => null)),
  );
  return gateways.find(Boolean) ?? null;
}
...
function probe(timeoutMs: number): Promise<string[]> {
  return new Promise((resolve) => {
    ...
    socket.on('message', (buffer, sender) => {
      if (buffer.length > 8192 || locations.size >= 16) return;
      ...
      if (location) {
        try {
          const target = validateGatewayUrl(location);
          if (target.hostname === sender.address) locations.add(target.href);
        } catch {
          /* Ignore unrelated or unsafe advertisements. */
        }
      }
    });
...
export async function describeGateway(location: string): Promise<Gateway | null> {
  const target = validateGatewayUrl(location);
  const response = await fetch(target, { signal: AbortSignal.timeout(4000), redirect: 'error' });
  if (!response.ok) return null;
  const xml = await limitedText(response);

  for (const block of xml.match(
    /<(?:[\w.-]+:)?service(?:\s[^>]*)?>[\s\S]*?<\/(?:[\w.-]+:)?service\s*>/gi,
  ) ?? []) {
    ...
    const control = validateGatewayUrl(new URL(controlPath.replace(/&amp;/g, '&'), location).href);
    if (control.hostname !== target.hostname) continue;
    return { serviceType, controlUrl: control.href };
  }
  return null;
}
...
export function gatewayFromControlUrl(controlUrl: string, serviceType?: string): Gateway {
  const target = validateGatewayUrl(controlUrl);
  const type = serviceType ?? 'urn:schemas-upnp-org:service:WANIPConnection:1';
  if (!WAN_SERVICES.includes(type)) throw new Error('Unsupported router service.');
  return { controlUrl: target.href, serviceType: type };
}
...
async function controlUrlReachable(url: string): Promise<boolean> {
  try {
    const parsed = validateGatewayUrl(url);
...
export async function discoverGatewayNear(hintUrl: string): Promise<Gateway | null> {
  ...
  try {
    const parsed = validateGatewayUrl(hintUrl);
  ...
  for (const port of [80, 1900, 5000, 5431, 49000]) add(port);
  ...
  for (let i = 0; i < ports.length; i += batchSize) {
    ...
    const results = await Promise.all(
      batch.map(async (port) => ({ port, ok: await tcpOpen(host, port) })),
    );
    open.push(...results.filter((row) => row.ok).map((row) => row.port));
  }

  // Bound recovery to at most eight open ports and one round of description requests.
  const gateways = await Promise.all(
    open
      .slice(0, 8)
      .flatMap((port) =>
        IGD_DESCRIPTION_PATHS.map((path) =>
          describeGateway(`http://${host}:${port}${path}`).catch(() => null),
        ),
      ),
  );
  return gateways.find(Boolean) ?? null;
...
async function soap(
  ...
  const body = args.map(([key, value]) => `<${key}>${escapeXml(String(value))}</${key}>`).join('');
  ...
  const verified = gatewayFromControlUrl(gateway.controlUrl, gateway.serviceType);
  const response = await fetch(verified.controlUrl, {
    method: 'POST',
    headers: {
...
export async function getPortMapping(
  ...
  try {
    ...
    // Only the explicit 'NoSuchEntryInArray' fault proves the port is free.
    // Authentication, unsupported-action and timeout failures must fail closed.
    if (error instanceof IgdError && error.code === 714) return null;
    throw error;
  }
}
 ...
 * sequence.
 */
export async function addPortMapping(gateway: Gateway, mapping: PortMapping): Promise<number> {
